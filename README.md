...
Đồ án/khóa luận tốt nghiệp — Trường Đại học Mỏ - Địa chất Hà Nội.
...
Dữ liệu đầu vào là 50 hồ sơ dân sự mẫu (`sample50.csv`, trích từ bộ Nemotron-Personas-Vietnam),
dùng làm proof-of-concept cho phương pháp luận; ứng dụng thực tế nhắm tới các nhóm người dùng
chuyên biệt hơn (định hướng phát triển tiếp theo, ngoài phạm vi khóa luận hiện tại).
...
## Ghi chú kỹ thuật
- `content_classifier.py` phân loại **văn bản đầu vào** (type + genre); `get_domain()` trong `community.py` phân loại **người dùng** — 2 mối quan tâm khác nhau dùng chung 1 cơ chế  weighted-keyword + threshold (0.6) + priority tie-break (margin 0.15).
...
- Cả hai dùng chung `pipeline/community.py`, `pipeline/worlds.py`, `pipeline/generation.py`,
  `pipeline/summarizer_gemini.py` — không duplicate logic nghiệp vụ.