...
Dùng gpt-oss-120b qua OpenAI-compatible endpoint (vLLM, RunAI).

  feature_coverage : tỷ lệ đặc trưng persona thực tế trong sample50.csv
                     được bao phủ bởi ít nhất một term trong ontology
  CQ_coverage      : tỷ lệ CQ có verdict PASS trong cq_validation_results_gpt.json

Quy trình đo feature_coverage (per branch):
  1. Dùng gpt-oss trích danh sách đặc trưng cụ thể từ toàn bộ 50 prose text
...
Cách dùng:
    ...
    python ontology_score_gpt.py --cq-results path/to/cq_validation_results_gpt.json
"""
...
BASE_URL   = "https://text-sum-gpt-oss-120b-runai-text-sum.runai-inference.cyberspace.vn"
...
def make_client() -> OpenAI:
    # verify=False: endpoint RunAI dùng self-signed cert (giống cq_validator_gpt.py)
    return OpenAI(
        api_key=API_KEY,
...
def retry_generate(client: OpenAI, prompt: str, max_tokens: int = 4096,
    ...
    """
    Gọi model gpt-oss-120b, chỉ retry khi gặp lỗi kết nối tầng transport
    (server này rate limit rất lớn nên không cần retry cho 429/rate-limit).
    """
    attempt = 0
    while True:
        try:
            ...
            print(f"\n  !! Lỗi kết nối ({type(e).__name__}). "
                  f"Thử lại lần {attempt}/{max_retries} sau {wait_time}s...")
...
def extract_features(client: OpenAI, branch_id: str,
    ...
    """
    Dùng gpt-oss trích danh sách đặc trưng cụ thể từ toàn bộ prose text của branch.
    Trả về list[str] đã de-duplicate.
    Chia prose thành batch để tránh JSON bị cắt giữa chừng khi output dài.
    """
    l1_str = "\n".join(f'- {t["name"]}: {t["def"][:120]}' for t in l1_terms)

    BATCH_SIZE = 15  # ~15 prose mỗi lần gọi, tránh output vượt max_tokens
    all_features = []
...
"""
        ...
        try:
            ...
            # Không dùng fallback split-by-line nữa — dòng rác sẽ làm lệch feature count.
            # Bỏ qua batch lỗi, không thêm gì vào all_features.
            continue
    ...
    return deduped[:80]  # cap tổng theo branch, giống quy tắc cũ
...
def semantic_match(client: OpenAI, branch_id: str,
    ...
    """
    Với các đặc trưng chưa match được string, hỏi gpt-oss xem
...
"""
    ...
    # Parse lỗi -> mặc định KHÔNG covered (conservative đúng nghĩa: không chắc
    # thì không tính là đã bao phủ, tránh thổi phồng feature_coverage giả tạo).
...
def measure_cq_coverage(cq_json_path: str) -> dict:
    if not os.path.exists(cq_json_path):
        raise FileNotFoundError(
            ...
            f"Kiểm tra lại tên file mà cq_validator_gpt.py thực sự xuất ra "
            f"(OUT_JSON trong file đó), hoặc truyền --cq-results trỏ đúng đường dẫn."
        )
