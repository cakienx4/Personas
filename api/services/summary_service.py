...
from pipeline.summarizer_gemini import SUMMARY_MODEL_NAME
...
async def _call_with_timeout(func, *args, timeout: float = STEP_TIMEOUT_SECONDS):
    try:
        ...
        raise TimeoutError(
            f"Gemini không phản hồi sau {timeout}s — có thể do quota hết hoặc mạng chậm."
        )


async def run_summarize(uuid: str, text: str, ratio: float = 0.7) -> dict:
    ...
    general_summary, general_meta = await _call_with_timeout(
        ...
        text,
        {"model": SUMMARY_MODEL_NAME, "contents": neutral_prompt, "config": {"temperature": 0.0}},
        "contents",
        ratio,
    )

    result, specific_meta = await _call_with_timeout(
        generate_specific_with_length_limit, row, text, g, client, ratio
    )
